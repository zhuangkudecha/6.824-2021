...
func NewMemoryStateConfigMachine() *MemoryConfigStateMachine {
...
func (cf *MemoryConfigStateMachine) Leave(gids []int) Err {
	...
	// delete the group and its shards
...
// from config get the group to shards map
...
func GetGIDWithMinimumShards(s2g map[int][]int) int {
	...
	sort.Ints(keys)
	// find the gid with minmum shards
